...
describe('Daemon Workflow Integration', () => {
  ...
  describe('daemon startup', () => {
    test(
      'should fail to start without SCHEDULE_ENABLED',
      done => {
    ...
    test(
      'should start successfully with proper configuration',
      done => {
        const daemon = spawn('node', ['src/main.js', 'daemon'], {
          cwd: path.resolve(__dirname, '../..'),
          env: {
  ...
  describe('daemon with initial execution', () => {
    test(
      'should run initial execution when RUN_ON_START is true',
      done => {
        const daemon = spawn('node', ['src/main.js', 'daemon'], {
          cwd: path.resolve(__dirname, '../..'),
          env: {
